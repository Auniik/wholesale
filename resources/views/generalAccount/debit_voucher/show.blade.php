                    ...
                    <div class="panel-heading no-print">
                        <div class="panel-heading-btn pull-right">
                            <div class="dropdown">
                                ...
                                @if ($debit_voucher->isPaymentType('partial'))
                                    @can('debit-voucher-list')
                                    <a href="{{route('debit-vouchers.index')}}"  class="btn btn-sm btn-success">Vouchers</a>
                                    @endcan
                                    @can('debit-voucher-create')
                                    <a class="btn btn-info btn-sm" href="{{ url('debit-vouchers/create') }}">Add New</a>
                                    @endcan
                                @else
                                    @can('advance-voucher-list')
                                    <a href="{{route('advance-payment-vouchers.index')}}"  class="btn btn-sm btn-success">Vouchers</a>
                                    @endcan
                                    @can('advance-voucher-create')
                                    <a class="btn btn-info btn-sm" href="{{ url('advance-payment-vouchers/create') }}">Add New</a>
                                    @endcan
                                @endif
                            </div>
                        </div>
                        ...
                        <table width="100%">
                            <tr>
                                <td style="border: 0;  text-align: left" width="40%">
                                    ...
                                    <br><br>
                                    <span style="font-size: 18px; color: #2f4f4f"><strong>INVOICE # {{$debit_voucher->id}}</strong></span>
                                </td>
...
{{--                                            <p><strong>Sales Executive :</strong> </p>--}}
                                            ...
                                            {{ optional($debit_voucher->party)->name }} <br>
                                            {{optional($debit_voucher->party)->mobile_number}} <br>
                                            {{--Bangladesh <br>--}}
                                            Phone: {{optional($debit_voucher->party)->telephone}} <br>Email: {{optional($debit_voucher->party)->email}} <br></td>
                                        <td class="meta-head">INVOICE #</td>
                                        <td>D-{{ str_pad($debit_voucher->id, 3, '0', STR_PAD_LEFT) }}</td>
                                    </tr>
                                    <tr>
                                        <td class="meta-head">Invoice Date</td>
                                        <td>{{$debit_voucher->date->format('d/m/Y')  }}</td>
                                    </tr>
                                    {{--<tr>--}}
                                    ...
                                    <tr>
                                        ...
                                        <td><div class="ref">{{$debit_voucher->ref_id}}</div></td>
                                    </tr>
                                    <tr>
                                        <td class="meta-head">Cheque No</td>
                                        <td><div class="ref">{{$debit_voucher->cheque_no}}</div></td>
                                    </tr>
                                    {{--<tr>--}}
                                    {{--<td class="meta-head">Amount Due</td>--}}
                                    {{--<td><div class="due">Tk. 31,420.00</div></td>--}}
                                    {{--</tr>--}}
                            ...
                            <table id="items">
                                ...
                                @php($tempPaid = 0)
                                @php($totalSectorAmount = 0)
                                @foreach($debit_voucher->sectors as $key => $sector)
                                    {{--{{dump(, 0)}}--}}
                                    <tr class="item-row">
                                        <? $amount = 0 ?>
                                        <td >{{$sector->chart_of_account->sector_name}}</td>
                                        <td >{{$sector->description}}</td>
                                        <td align="right">Tk. {{number_format($sector->amount, 2)}}</td>
                                        {{--<td align="right">Tk. {{$sector->paymentDue}}</td>--}}
                                        {{--<td align="right">Tk. {{number_format(abs($sector->paid), 2)}} TK</td>--}}
{{--                                        <td align="right">{{number_format(abs(!$sector->paid ? optional($sector->sectorPayment)->paid_amount : $sector->paid), 2)}} TK</td>--}}
                                        <?php
//                                            $totalSectorAmount =
                                            $tempPaid += $sector->sectorPayment->paid_amount ?? 0;
                                        ?>
                                    </tr>
                                @endforeach
                                {{--<tr>--}}
                                ...
                                <tr>
                                    ...
                                    <td class="total-value"><div class="due">Tk. {{ number_format(abs(!$debit_voucher->sectors->sum('paid') ? $tempPaid : $debit_voucher->sectors->sum('paid')), 2 )}}</div></td>
                                </tr>
                                <tr>
                                    {{--<td class="blank"> </td>--}}
                                    {{--<td class="blank"> </td>--}}
                                    ...
                                    <td class="total-value balance">
                                        <div class="due">
                                            @if ($debit_voucher->approved_at && $debit_voucher->confirmed_at)
                                                Tk.{{ number_format(abs ($debit_voucher->sectors->sum('paymentDue')), 2 )}}
                                                @else

                                                Tk {{number_format($debit_voucher->sectors->sum('amount') -
                                                $tempPaid, 2)}}
                                            @endif
                            ...
                            <div class="print-footer" style="margin-top: 40px;overflow: hidden;width: 100%;padding: 0 10px;">
                                <div class="sign" style="width: 100%; overflow: hidden;">
                                    <div class="company_sign" style="width: 25%; float: left;">
                                    ...
                                    <div class="company_sign" style="width: 25%; float: left;">
                                        <h5 style="width:50%; margin: 0 auto; padding: 10px 0;text-align: center;">{{$debit_voucher->user->name}}</h5>
                                    ...
                                    <div class="company_sign" style="width: 25%; float: left;">
                                        <h5 style="width:50%; margin: 0 auto; padding: 10px 0;text-align: center;">
                                            {{optional($debit_voucher->confirmedBy)->name}} &nbsp;
                                            {{--<span>{{optional($debit_voucher->approvedByEmployee)->designation->name ?? ''--}}
                                            {{--}}</span>--}}
                                        </h5>
                                        <h5 style="width:50%;margin: 0 auto;border-top: 1px solid #000;padding: 10px
                                        0;text-align: center;">Verified By</h5>
                                    </div>
                                    <div class="company_sign" style="width: 25%; float: left;">
                                        <h5 style="width:50%; margin: 0 auto; padding: 10px 0;text-align: center;">{{optional($debit_voucher->approvedBy)->name}} &nbsp;</h5>
...
{{--                                        <p class="design_band pull-right">Powered By: <a href="#" > Smart Software Inc.</a></p>--}}
                            ...
                            <div id="terms">
                                {{--<h5>Terms</h5>--}}
                                {{--<p>Please pay to bKash No: 01777 333 677 (personal)<br>Or<br>Account Name: DataHost IT<br>
