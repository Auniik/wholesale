            ...
            <div class="col-md-12">
                <div class="panel panel-default">
                    <div class="panel-heading">
                        ...
                        <h4 class="panel-title">Confirm Debit Voucher </h4>
                    </div>
                    <div class="panel-body" id="tabArea">
                        @if(!$debit_voucher->approved_at)
                            @if($debit_voucher->confirmed_at)
                                <h3>Debit Voucher already confirmed by {{$debit_voucher->confirmedBy->name}}</h3>
                            @else
                                <form action="{{action('GeneralPaymentController@updateConfirm', $debit_voucher)}}"
                                      method="post">
                                    @csrf
                                    @method('put')
                                            ...
                                            <table class="table table-bordered table-hover" id="table_auto">
                                                ...
                                                @foreach($debit_voucher->sectors ?? [] as $sector)
                                                    <tr class="record-row">
                                                        ...
                                                        <?php
                                                        $paid += abs(!$sector->paid ? optional($sector->sectorPayment)->paid_amount : $sector->paid)
                                                        ?>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-group aside_system">
                                                ...
                                                <div class="input-group col-md-8">
                                                    <div class="input-group-addon currency">৳</div>
                                                    <input value="{{$debit_voucher->sectors->sum('amount') - $paid}}"
                                                           type="number" min="0" step="any" class="form-control amountDue" tabindex="-1"  id="amountDue" placeholder="Amount Due" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" readonly >
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6 col-lg-offset-3">
                                            <label>&nbsp;</label>
                                            <button type="submit" class="btn btn-success col-sm-12">Verify
                                                Payment</button>
                                        </div>
                                    </div>
                                </form>
                        ...
                        @else
                            <h3>Voucher Already APPROVED by {{$debit_voucher->approvedBy->name}}</h3>
                        @endif
                    </div>
                </div>
            </div>
